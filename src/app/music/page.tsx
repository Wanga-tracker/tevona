...
export default function MusicDownloader() {
  ...
  const [nowPlaying, setNowPlaying] = useState<{ type: "audio" | "video"; id: string; idx: number } | null>(null);
  ...
  const handleSearch = async (q?: string) => {
    ...
    try {
      const res = await fetch(
        `https://tevona-api.onrender.com/music/search?q=${encodeURIComponent(searchTerm)}`
      );
          ...
          >
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-3 mt-3">
                <button
                  onClick={() => setNowPlaying({ type: "audio", id: song.videoId, idx })}
                  className="px-4 py-2 bg-slate-700/60 rounded-lg hover:bg-slate-600 transition"
                >
                  ▶ Play Audio
                </button>
                <button
                  onClick={() => setNowPlaying({ type: "video", id: song.videoId, idx })}
                  className="px-4 py-2 bg-slate-700/60 rounded-lg hover:bg-slate-600 transition"
                >
                  ▶ Play Video
                </button>
                ...
                <div className="mt-4">
                  {nowPlaying.type === "audio" ? (
                    <audio
                      ...
                      autoPlay
                      onEnded={() => {
                        const nextSong = results[nowPlaying.idx + 1];
                        if (nextSong) {
                          setNowPlaying({ type: "audio", id: nextSong.videoId, idx: nowPlaying.idx + 1 });
                        }
                      }}
                      className="w-full"
                    />
