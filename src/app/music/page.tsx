...
const SEARCH_API = "https://tevona-api.onrender.com"; // your Render lightweight API
const DOWNLOAD_API = "http://fi9.bot-hosting.net:22097"; // bot-hosting backend
...
export default function MusicDownloader() {
  ...
  const handleSearch = async (q?: string) => {
    ...
    setLoading(true);
    const data = await fetcher<Song[]>(
      `${SEARCH_API}/music/search?q=${encodeURIComponent(searchTerm)}`
    );
    setResults(data || []);
    setLoading(false);
  };
  ...
  const handleDownload = (videoId: string, type: "audio" | "video") => {
    window.open(
      `${DOWNLOAD_API}/music/download?id=${videoId}&type=${type}`,
      "_blank"
    );
  };
      ...
      <div className="grid gap-6 max-w-4xl mx-auto">
        {results.map((song, idx) => (
          <motion.div
            ...
            className="flex flex-col md:flex-row items-center md:items-start gap-4 p-4 rounded-2xl bg-slate-800/30 backdrop-blur-lg border border-slate-700/50 shadow-lg hover:shadow-xl"
          >
            <img
              src={song.thumbnail}
              alt={song.title}
            ...
            <div className="flex-1">
              ...
              {nowPlaying?.id === song.videoId && (
                <div className="mt-4">
                  {nowPlaying.type === "audio" ? (
                    <audio
                      src={`${DOWNLOAD_API}/music/download?id=${song.videoId}&type=audio`}
                      controls
                      autoPlay
                      className="w-full"
                    />
